-- Drop leagues.points_for_wrong: a wrong prediction is always worth 0 and was
-- never meaningfully league-configurable. Hardcode the literal 0 in
-- evaluate_match_bets, then remove the column.
--
-- points_for_exact_score (3) stays as-is; correct result stays hardcoded to 1.
-- Favorite-team doubling (00020) is preserved.
...
BEGIN
  ...
  FOR bet_record IN
    SELECT b.*, l.points_for_exact_score, p.favorite_nation_id
    FROM bets b
...
ALTER TABLE leagues DROP COLUMN IF EXISTS points_for_wrong;
