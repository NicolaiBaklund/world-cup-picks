-- ============================================
-- LEAGUES TABLE
-- Private groups where friends compete
-- ============================================

CREATE TABLE leagues (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL,
  description TEXT,
  creator_id UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
  
  -- Invite system
  code CHAR(6) UNIQUE NOT NULL,  -- Shareable invite code
  
  -- Settings
  points_for_correct INT DEFAULT 1,
  points_for_wrong INT DEFAULT 0,
  points_for_exact_score INT DEFAULT 3,  -- Future: bonus for exact score
  allow_late_join BOOLEAN DEFAULT true,
  is_public BOOLEAN DEFAULT false,
  max_members INT DEFAULT 50,
  
  -- Metadata
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- Index for invite code lookups
CREATE INDEX idx_leagues_code ON leagues(code);

-- Index for creator lookups
CREATE INDEX idx_leagues_creator ON leagues(creator_id);

-- Trigger to auto-update updated_at
CREATE TRIGGER leagues_updated_at
  BEFORE UPDATE ON leagues
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at();

-- Function to generate unique invite code
CREATE OR REPLACE FUNCTION generate_league_code()
RETURNS TRIGGER AS $$
DECLARE
  new_code CHAR(6);
  code_exists BOOLEAN;
BEGIN
  IF NEW.code IS NULL OR NEW.code = '' THEN
    LOOP
      new_code := upper(substr(md5(random()::text), 1, 6));
      SELECT EXISTS(SELECT 1 FROM leagues WHERE code = new_code) INTO code_exists;
      EXIT WHEN NOT code_exists;
    END LOOP;
    NEW.code := new_code;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER generate_league_code_trigger
  BEFORE INSERT ON leagues
  FOR EACH ROW
  EXECUTE FUNCTION generate_league_code();

-- Row Level Security
ALTER TABLE leagues ENABLE ROW LEVEL SECURITY;

-- Anyone can view public leagues, members can view private leagues
CREATE POLICY "Public leagues are viewable by everyone"
  ON leagues FOR SELECT
  USING (is_public = true OR creator_id = auth.uid() OR 
         EXISTS (SELECT 1 FROM league_members WHERE league_id = id AND user_id = auth.uid()));

-- Only authenticated users can create leagues
CREATE POLICY "Authenticated users can create leagues"
  ON leagues FOR INSERT
  WITH CHECK (auth.uid() = creator_id);

-- Only creator can update league
CREATE POLICY "Creator can update league"
  ON leagues FOR UPDATE
  USING (auth.uid() = creator_id);

-- Only creator can delete league
CREATE POLICY "Creator can delete league"
  ON leagues FOR DELETE
  USING (auth.uid() = creator_id);

COMMENT ON TABLE leagues IS 'Betting leagues/groups for friends to compete';
COMMENT ON COLUMN leagues.code IS 'Shareable 6-character invite code';
