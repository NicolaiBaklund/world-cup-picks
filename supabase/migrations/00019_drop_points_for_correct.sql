-- Drop leagues.points_for_correct: the "correct result, wrong score" tier is
-- always worth 1 point and was never league-configurable. Hardcode the literal 1
-- in evaluate_match_bets, then remove the column.
--
-- points_for_exact_score (3) and points_for_wrong (0) stay as-is.

-- Rewrite evaluate_match_bets to stop reading l.points_for_correct.
...
ALTER TABLE leagues DROP COLUMN IF EXISTS points_for_correct;
